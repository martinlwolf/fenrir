...
5. **Business-rules pre-check (before asking the user)**: this project keeps an
   `analista-funcional` agent (`.claude/agents/analista-funcional.md`) that knows
   `business_rules/` in depth. Before presenting any queued question to the user, check
   whether it falls inside that domain (roles, project types, tokens, milestone
   lifecycle, funding, commission, edge cases — see `business_rules/index.md`); if it
   doesn't, skip straight to step 6.
    - If it does, dispatch `analista-funcional` via the `Agent` tool with the exact
      question, instructing it to reread `business_rules/index.md` and the relevant
      files in full (never from memory) and answer ONLY if the documents already
      settle it, with a citation (file, and section/quote if it helps).
    - If the agent returns a confident, cited answer: treat the question as resolved
      without asking the user. Record it directly per the integration rules in step 7
      below — `- Q: <question> → A: <answer> (resuelto vía business_rules/, ver
      <archivo>)` — and remove it from the queue. This does **not** count against the
      5-question quota; that quota only covers questions actually asked to the user.
    - If the agent reports it doesn't know, that the documents don't settle it, or that
      it's listed in `business_rules/decisiones-pendientes.md` as open: leave the
      question in the queue and proceed to step 6 to ask the user normally — never
      guess a business-rules answer on the user's behalf.

6. Sequential questioning loop (interactive) — covers questions not already resolved
   by the business-rules pre-check in step 5:
...
- Number of questions asked & answered to the user, AND separately the number
  resolved by the `analista-funcional` business-rules pre-check (step 5) without
  involving the user — list each of those with its citation.
