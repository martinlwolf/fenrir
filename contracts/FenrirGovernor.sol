...
contract FenrirGovernor is ERC1155 {
    enum VotingMode { ByToken, OneWalletOneVote }
    enum ProposalKind { ArbiterElection, Milestone, SaleOffer }
    enum ProposalStatus { Active, AwaitingArbiter, Resolved }
    enum VoteResult { None, Approved, Rejected }
    ...
    event ProposalCreated(uint256 indexed proposalId, ProposalKind kind, uint256 refId, uint256 snapshotBlock, uint256 deadline);
    ...
    modifier onlyArbiter() {
        require(msg.sender == arbiter && arbiter != address(0), "FenrirGovernor: only arbiter");
        _;
    }
    ...
    function proposeArbiterElection() external onlyProject returns (uint256 proposalId) {
        proposalId = _createProposal(ProposalKind.ArbiterElection, 0);
    }

    function proposeMilestone(uint256 milestoneId) external onlyProject returns (uint256 proposalId) {
        proposalId = _createProposal(ProposalKind.Milestone, milestoneId);
    }

    function proposeSaleOffer(uint256 offerId) external onlyProject returns (uint256 proposalId) {
        proposalId = _createProposal(ProposalKind.SaleOffer, offerId);
    }

    function _createProposal(ProposalKind kind, uint256 refId) internal returns (uint256 proposalId) {
        ...
        emit ProposalCreated(proposalId, kind, refId, snapshotBlock, p.deadline);
    }
    ...
    function castVote(uint256 proposalId, bool support) external {
        ...
        require(p.kind == ProposalKind.Milestone || p.kind == ProposalKind.SaleOffer, "FenrirGovernor: not a binary vote");
    ...
    function castDeveloperSaleVote(uint256 proposalId, bool support) external {
        Proposal storage p = _activeProposal(proposalId);
        require(p.kind == ProposalKind.SaleOffer, "FenrirGovernor: not a sale offer");
        require(msg.sender == IFenrirProjectCallback(project).developer(), "FenrirGovernor: only developer");
    ...
    function castElectionVote(uint256 proposalId, address candidate) external {
        Proposal storage p = _activeProposal(proposalId);
        require(p.kind == ProposalKind.ArbiterElection, "FenrirGovernor: not an election");
        require(candidate != IFenrirProjectCallback(project).developer(), "FenrirGovernor: developer cannot be candidate");
        uint256 weight = _votingWeight(msg.sender, p.snapshotBlock);
        require(weight > 0, "FenrirGovernor: no voting power");
        _registerVote(proposalId, p, msg.sender, weight);
        ...
        if (newTotal > p.leadingVotes) {
            ...
            p.tie = false;
        } else if (newTotal == p.leadingVotes && candidate != p.leadingCandidate) {
            p.tie = true;
        }
    ...
    function _registerVote(uint256 proposalId, Proposal storage p, address voter, uint256 weight) internal {
    ...
    function _afterVoteCast(uint256 proposalId, Proposal storage p) internal {
        if (p.totalPowerAtSnapshot > 0 && p.weightVoted >= p.totalPowerAtSnapshot) {
            _resolve(proposalId, p);
        }
    }

    function _activeProposal(uint256 proposalId) internal view returns (Proposal storage p) {
        p = proposals[proposalId];
        require(p.status == ProposalStatus.Active, "FenrirGovernor: not active");
    ...
    function resolve(uint256 proposalId) external {
        Proposal storage p = proposals[proposalId];
        require(p.status == ProposalStatus.Active, "FenrirGovernor: not active");
        require(block.timestamp > p.deadline, "FenrirGovernor: voting still open");
        _resolve(proposalId, p);
    }

    function _quorumReached(Proposal storage p) internal view returns (bool) {
        return p.totalPowerAtSnapshot > 0 &&
            p.weightVoted * BPS_DENOMINATOR >= QUORUM_BPS * p.totalPowerAtSnapshot;
    }

    function _resolve(uint256 proposalId, Proposal storage p) internal {
        bool quorumReached = _quorumReached(p);

        if (!quorumReached && p.kind != ProposalKind.ArbiterElection && !p.extended) {
            p.extended = true;
            p.deadline = block.timestamp + VOTING_PERIOD;
        ...
        VoteResult result = p.votesFor * BPS_DENOMINATOR >= APPROVAL_THRESHOLD_BPS * p.weightVoted
            ? VoteResult.Approved
            : VoteResult.Rejected;
    ...
    function arbiterDecide(uint256 proposalId, bool approve) external onlyArbiter {
        Proposal storage p = proposals[proposalId];
        require(p.status == ProposalStatus.AwaitingArbiter, "FenrirGovernor: not awaiting arbiter");
        _finalize(proposalId, p, approve ? VoteResult.Approved : VoteResult.Rejected);
    }
    ...
    /// de resolverla.
    function _resolveByRandom(uint256 proposalId, Proposal storage p, bool quorumReached) internal {
        address[] memory pool = quorumReached ? _tiedCandidates(proposalId, p.leadingVotes) : _eligibleInvestors();
        if (pool.length == 0) {
        ...
        uint256 randomSeed = uint256(
            keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp, proposalId))
        );
        p.leadingCandidate = pool[randomSeed % pool.length];
        _finalize(proposalId, p, VoteResult.Approved);
    }

    function _tiedCandidates(uint256 proposalId, uint256 leadingVotes) internal view returns (address[] memory) {
    ...
    function _finalize(uint256 proposalId, Proposal storage p, VoteResult result) internal {
        ...
        if (p.kind == ProposalKind.ArbiterElection) {
            ...
            IFenrirProjectCallback(project).onMilestoneResolved(p.refId, result == VoteResult.Approved);
        } else {
            IFenrirProjectCallback(project).onSaleOfferResolved(p.refId, result == VoteResult.Approved);
        }
    }
    ...
    function _votingWeight(address account, uint256 snapshotBlock) internal view returns (uint256) {
    ...
    function _totalPowerAtSnapshot(uint256 snapshotBlock, ProposalKind kind) internal view returns (uint256) {
    ...
    function _holderCountAtSnapshot(uint256 snapshotBlock) internal view returns (uint256) {
        uint256 count = token.holdersCount();
        uint256 n = 0;
        for (uint256 i = 0; i < count; i++) {
