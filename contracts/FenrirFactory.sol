...
contract FenrirCompletionCertificate is ERC721 {
    ...
    constructor(address factory_) ERC721("Fenrir Completion Certificate", "FENRIR-OK") {
        factory = factory_;
    }

    function mintTo(address developer_) external returns (uint256 tokenId) {
        require(msg.sender == factory, "FenrirCompletionCertificate: only factory");
        tokenId = nextTokenId++;
        _safeMint(developer_, tokenId);
    }

    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        require(_ownerOf(tokenId) == address(0), "FenrirCompletionCertificate: soulbound");
...
contract FenrirFailureCertificate is ERC721 {
    ...
    constructor(address factory_) ERC721("Fenrir Failure Certificate", "FENRIR-FAIL") {
        factory = factory_;
    }

    function mintTo(address developer_) external returns (uint256 tokenId) {
        require(msg.sender == factory, "FenrirFailureCertificate: only factory");
        tokenId = nextTokenId++;
        _safeMint(developer_, tokenId);
    }

    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        require(_ownerOf(tokenId) == address(0), "FenrirFailureCertificate: soulbound");
        return super._update(to, tokenId, auth);
    }
}
...
contract FenrirFactory {
    ...
    mapping(address => bool) public isFenrirProject;
    ...
    function registerDeveloper(string calldata razonSocial, string calldata cuit) external {
        require(bytes(razonSocial).length > 0, "FenrirFactory: razonSocial required");
        require(_isValidCuit(cuit), "FenrirFactory: invalid CUIT");
        address boundWallet = cuitToWallet[cuit];
        require(boundWallet == address(0) || boundWallet == msg.sender, "FenrirFactory: CUIT bound to another wallet");

        developers[msg.sender] = DeveloperIdentity({ razonSocial: razonSocial, cuit: cuit, registered: true });
    ...
    function createProject(
        ...
        require(developers[msg.sender].registered, "FenrirFactory: developer not registered");
        require(
            projectType == FenrirProject.ProjectType.Civic || votingMode == FenrirGovernor.VotingMode.ByToken,
            "FenrirFactory: Investment requires token-weighted voting"
        );

        FenrirToken token = new FenrirToken(tokenName, tokenSymbol);
        FenrirGovernor governor = new FenrirGovernor(address(token), votingMode);
        ...
        emit ProjectCreated(projectAddress, address(token), address(governor), msg.sender, projectType);
    }

    modifier onlyKnownProject() {
        require(isFenrirProject[msg.sender], "FenrirFactory: only a Fenrir project");
        _;
    }

    function recordSuccess(address developerAddr) external onlyKnownProject {
        completionCertificate.mintTo(developerAddr);
    }

    function recordFailure(address developerAddr) external onlyKnownProject {
